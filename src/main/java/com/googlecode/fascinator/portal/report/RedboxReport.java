...
/*	private String processReportCriteria(JsonObject queryFilters, int index) {
		...
		criteriaValue = criteriaValue.replace("-", "\\-");
		if (queryFilters.get("report-criteria." + index + ".match_contains") != null) {
	...
	/**
	 * Returns the report criteria - not including the date range and
	 * published/notpublished
	 * 
	 * @return
	 */
	public SearchCriteriaListing getCriteria() {
		return new SearchCriteriaListing(this, config, this.strDateFormat,this.solrDateFormat);
	}
}
